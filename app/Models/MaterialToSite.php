...
class MaterialToSite extends BaseModel
{
    /**
     * appends
     *
    ...
    public function getIsPartialCompleteAttribute()
    {
        ...
        $rec_qty = $details->sum(function ($i) {
            return $i->stocks->sum(function ($i) {
        ...
        $child_details = collect($this->childs()->where('status', 1)->with('details')->get())->sum(function ($i) {
            $d = $i->details->sum(function ($e) {
                return $e->stocks->sum(function ($e) {
    ...
    {
        return $this->hasMany(MaterialToSiteDetail::class, 'material_to_site_id', 'id');
    }
    ...
    {
        return $this->hasMany(__CLASS__, 'material_to_site_id', 'id');
    }
    ...
    {
        return $this->belongsTo(__CLASS__, 'material_to_site_id', 'id');
    }
    ...
    {
        return $this->hasOne(UsedMaterialReturn::class);
    }
}
